...
//			map.put("Recovery", false);
	...
	private void recover() {
		...
		if(null != accountSettingRecoveryProcessor) {
			List<AccountSetting> list = accountSettingRecoveryProcessor.recover();
		...
		if(null != positionRecoveryProcessor) {
			List<OpenPosition> list1 = positionRecoveryProcessor.recoverOpenPositions();
			log.info("Open positions loaded: " + list1.size());
			List<ClosedPosition> list2 = positionRecoveryProcessor.recoverClosedPositions();
