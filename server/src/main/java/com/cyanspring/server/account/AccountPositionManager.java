...
public class AccountPositionManager implements IPlugin {
	...
	private IQuoteFeeder quoteFeeder = new IQuoteFeeder() {
		...
		public Quote getQuote(String symbol) {
			...
			if (null == quote) {
				eventManager.sendEvent(new QuoteSubEvent(
						AccountPositionManager.ID, null, symbol));
			}
			return quote;
		}
	};
	...
	private AsyncEventProcessor eventProcessor = new AsyncEventProcessor() {
		...
		@Override
		public void subscribeToEvents() {
			subscribeToEvent(UserCreateAndLoginEvent.class, null); // for
																	// Facebook,
																	// QQ,
																	// WeChat
	...
	private Date getScheuledDate() throws AccountException {
		if (dailyExecTime == null || dailyExecTime.length() == 0) {
			throw new AccountException(
					"didn't set scheduled daily execution time");
		}
		...
		Date scheduledToday = TimeUtil.getScheduledDate(cal, now, nHour, nMin,
				nSecond);
	...
	private void scheduleDayEndEvent() {
		try {
			...
			scheduleManager.scheduleTimerEvent(date, eventProcessor,
					dayEndEvent);
	...
	public void init() throws Exception {
		...
		perfDataUpdate = new PerfDurationCounter("Dynamic data update",
				perfUpdateInterval);
		perfDataRm = new PerfDurationCounter("Risk management", perfRmInterval);
		perfFqyAccountUpdate = new PerfFrequencyCounter(
				"Dynamic account update", 30000);
		perfFqyPositionUpdate = new PerfFrequencyCounter(
				"Dynamic position update", 30000);
		...
		scheduleManager.scheduleRepeatTimerEvent(jobInterval, timerProcessor,
				timerEvent);
	...
	IPositionListener positionListener = new IPositionListener() {
		...
		boolean dynamicDataHasChanged(Account account) {
			...
			if (last == null || account == null
					|| last.getMargin() != account.getMargin()
					|| last.getUrPnL() != account.getUrPnL()
					|| last.getCashAvailable() != account.getCashAvailable()
					|| last.getPnL() != account.getPnL()
					|| last.getUrLastPnL() != account.getUrLastPnL()) {
				try {
		...
		boolean dynamicDataHasChanged(OpenPosition position) {
			...
			if (last == null || position == null
					|| last.getPnL() != position.getPnL()
					|| last.getAcPnL() != position.getAcPnL()
		...
		public void onRemoveDetailOpenPosition(OpenPosition position) {
			eventManager.sendEvent(new PmRemoveDetailOpenPositionEvent(
					PersistenceManager.ID, position));

		}

		@Override
		public void onUpdateDetailOpenPosition(OpenPosition position) {
			eventManager.sendEvent(new PmUpdateDetailOpenPositionEvent(
					PersistenceManager.ID, position));
		}
		...
		public void onOpenPositionUpdate(OpenPosition position) {
			try {
				positionUpdates.put(position.getId(), position);
				eventManager.sendGlobalEvent(new OpenPositionUpdateEvent(
						position.getAccount(), null, position));
			} catch (Exception e) {
				log.error(e.getMessage(), e);
			}
		}
		...
		public void onOpenPositionDynamiceUpdate(OpenPosition position) {
			try {
				if (sendDynamicPositionUpdate
						&& dynamicDataHasChanged(position)) {
					perfFqyPositionUpdate.count();
					eventManager
							.sendRemoteEvent(new OpenPositionDynamicUpdateEvent(
									position.getAccount(), null, position));
				}
			} catch (Exception e) {
				log.error(e.getMessage(), e);
		...
		public void onClosedPositionUpdate(ClosedPosition position) {
			try {
				eventManager.sendGlobalEvent(new ClosedPositionUpdateEvent(
						position.getAccount(), null, position));
			} catch (Exception e) {
				log.error(e.getMessage(), e);
			}
		...
		public void onAccountUpdate(Account account) {
			try {
				accountUpdates.put(account.getId(), account.clone());
				eventManager.sendRemoteEvent(new AccountUpdateEvent(account
						.getId(), null, account));
				eventManager.sendEvent(new PmUpdateAccountEvent(
						PersistenceManager.ID, null, account));
			} catch (Exception e) {
				log.error(e.getMessage(), e);
			}
		}
		...
		public void onAccountDynamicUpdate(Account account) {
			try {
				if (dynamicDataHasChanged(account)) {
					...
					eventManager.sendRemoteEvent(new AccountDynamicUpdateEvent(
							account.getId(), null, account));
				}
			} catch (Exception e) {
				log.error(e.getMessage(), e);
			}
	...
	public void processRateConverterRequestEvent(RateConverterRequestEvent event) {
		try {
			RateConverterReplyEvent reply = new RateConverterReplyEvent(
					event.getKey(), event.getSender(), fxConverter);
	...
	public void processOverAllPositionRequestEvent(
			OverAllPositionRequestEvent event) {
		...
		if (null == event.getAccountIdList()) {
			...
			if (accountList.size() > limitUser) {
				try {
					OverAllPositionReplyEvent reply = new OverAllPositionReplyEvent(
							event.getKey(), event.getSender(), false,
							"over limit user counts:" + limitUser, null, null);
					eventManager.sendRemoteEvent(reply);
					log.info("OverAllPositionReplyEvent sent fail: over limit user counts:"
							+ limitUser);
				} catch (Exception e) {
					log.error(e.getMessage(), e);
				}
	...
	private void asyncSendOverallPosition(
			final OverAllPositionRequestEvent event,
			final List<Account> accounts) {

		Thread thread = new Thread(new Runnable() {

			private void sendOverAllPositionReplyEvent(
					List<OpenPosition> openPositionList,
					List<ClosedPosition> closedPositionList) {
				try {
					OverAllPositionReplyEvent reply = new OverAllPositionReplyEvent(
							event.getKey(), event.getSender(), true, "",
							openPositionList, closedPositionList);

					eventManager.sendRemoteEvent(reply);
					log.info("OverAllPositionReplyEvent sent: op:{} cp:{}",
							openPositionList.size(), closedPositionList.size());
			...
			public void run() {
				...
				for (int i = 0; i < accounts.size(); i++) {
					...
					List<OpenPosition> tempOpList = positionKeeper
							.getOverallPosition(account);
					List<ClosedPosition> tempCpList = positionKeeper
							.getClosedPositions(account.getId());
					...
					for (OpenPosition op : tempOpList) {
						...
						if (positionCount % asyncSendBatch == 0) {
							sendOverAllPositionReplyEvent(openPositionList,
									closedPositionList);
							positionCount = 0;
	...
	public void processUpdateOpenPositionPriceEvent(
			UpdateOpenPositionPriceEvent event) {
		String symbol = event.getSymbol();
		String account = event.getAccount();
		double price = event.getPrice();
		log.info("processUpdateOpenPositionPriceEvent, account: " + account
				+ ", symbol: " + symbol + ", price: " + price);
		if (!StringUtils.hasText(symbol) || !StringUtils.hasText(account)
				|| PriceUtils.EqualLessThan(price, 0)) {
			log.error("processUpdateOpenPositionPriceEvent fail, account: "
					+ account + ", symbol: " + symbol + ", price: " + price);
			return;
		}
	...
	public void processManualClosePositionRequestEvent(
			ManualClosePositionRequestEvent event) {
		OpenPosition position = event.getOpenPosition();
		double price = event.getPrice();
		log.info("processManualClosePositionRequestEvent, " + "account: "
				+ position.getAccount() + ", symbol: " + position.getSymbol()
				+ ", price: " + price);
	...
	public void processAddCashEvent(AddCashEvent event) {
		log.info("process AddCashEvent, account: {}, cash: {}",
				event.getAccount(), event.getCash());
		...
		PmAddCashEvent pmAddCashEvent = new PmAddCashEvent(null, null, account,
				event.getCash(), AuditType.DEPOSIT);
	...
	public void processUserLoginEvent(UserLoginEvent event) {
		log.debug("Received UserLoginEvent: " + event.getUserId() + ", txid: "
				+ event.getTxId() + ", k: " + event.getKey());
		eventManager.sendEvent(new PmUserLoginEvent(PersistenceManager.ID,
				event.getReceiver(), userKeeper, accountKeeper, event));
	}
	...
	public void processCreateUserEvent(CreateUserEvent event) {
		...
		if (null != userKeeper && null != accountKeeper) {
			try {
				...
				Account account = new Account(defaultAccountId, event.getUser()
						.getId());
				accountKeeper.setupAccount(account);

				eventManager.sendEvent(new PmCreateUserEvent(
						PersistenceManager.ID, null, user, event, Arrays
								.asList(account)));
			} catch (UserException ue) {
				message = MessageLookup.buildEventMessage(
						ue.getClientMessage(), ue.getMessage());
				ok = false;
			} catch (AccountException ae) {
				message = MessageLookup.buildEventMessage(
						ae.getClientMessage(), ae.getMessage());
				ok = false;
			}
		} else {
			ok = false;
			message = MessageLookup.buildEventMessage(
					...
					"System doesn't support user creation");
		}

		log.info("processCreateUserEvent: " + event.getUser() + ", " + ok
				+ ", " + message);

		if (!ok) {
			try {
				eventManager.sendRemoteEvent(new CreateUserReplyEvent(event
						.getKey(), event.getSender(), user, ok, message, event
						.getTxId()));
			} catch (Exception e) {
				log.error(e.getMessage(), e);
			}
	...
	public void processUserCreateAndLoginEvent(UserCreateAndLoginEvent event) {
		log.debug("Received UserCreateAndLoginEvent: "
				+ event.getUser().getId());
		boolean ok = true;
		User user = event.getUser();
		String message = "";
		...
		if (null != userKeeper && null != accountKeeper) {
			try {
				...
				if (Strings.isNullOrEmpty(user.getId())
						|| !userKeeper.userExists(user.getId())) {
					String defaultAccountId = getDefaultAccountId(user);

					user.setDefaultAccount(defaultAccountId);
					Account account = new Account(defaultAccountId, event
							.getUser().getId());
					accountKeeper.setupAccount(account);
					...
					// user not exist
					eventManager.sendEvent(new PmUserCreateAndLoginEvent(
							PersistenceManager.ID, event.getReceiver(), user,
							event, userKeeper, accountKeeper, Arrays
									.asList(account)));
				} else {
					// user exist
					eventManager.sendEvent(new PmUserCreateAndLoginEvent(
							PersistenceManager.ID, event.getReceiver(), null,
							event, userKeeper, accountKeeper, null));
				}
				...
				message = MessageLookup.buildEventMessage(
						ErrorMessage.ACCOUNT_NOT_EXIST, ae.getMessage());
				ok = false;
			}
		} else {
			ok = false;
			...
			message = MessageLookup.buildEventMessage(
					...
					"System not yet Ready for Authentication");

		}

		log.info("processUserCreateAndLoginEvent: " + event.getUser() + ", "
				+ ok + ", " + message);

		if (!ok) {
			try {
				eventManager.sendRemoteEvent(new UserCreateAndLoginReplyEvent(
						event.getKey(), event.getSender(), user, null, null,
						false, event.getOriginalID(), message, event.getTxId(),
						false));
			} catch (Exception e) {
				log.error(e.getMessage(), e);
			}
			...
			if (!accountKeeper.accountExists(user.getId() + "-"
				...
				if (accountKeeper.accountExists(defaultAccountId)) {
					throw new UserException(
							"Cannot create default account for user: "
									+ user.getId() + ", last try: "
									+ defaultAccountId,
	...
	private void checkUserType(User user) throws UserException {
		if (null == user.getUserType()
				|| (!user.getUserType().equals(UserType.FACEBOOK)
						&& !user.getUserType().equals(UserType.QQ)
						&& !user.getUserType().equals(UserType.WECHAT) && !user
						.getUserType().equals(UserType.TWITTER))) {
			throw new UserException("Cannot create user by wrong UserType",
					ErrorMessage.WRONG_USER_TYPE);
		}
	}
	...
	public void processChangeAccountStateRequestEvent(
			ChangeAccountStateRequestEvent event) {
		...
		check: {
			...
			if (!StringUtils.hasText(id)) {
				message = MessageLookup.buildEventMessage(
						ErrorMessage.CHANGE_ACCOUNT_STATE_FAILED,
						"id is empty!");
				break check;
			}
		...
		if (!StringUtils.hasText(message)) {
			...
			} else if (AccountState.FROZEN.equals(newState)
					|| AccountState.TERMINATED.equals(newState)) {
				TradingUtil.closeAllPositoinAndOrder(account, positionKeeper,
						eventManager, true, OrderReason.CompanyManualClose,
						riskOrderController);
				setAccountState(account, newState, event);
			}
		} else {
			ChangeAccountStateReplyEvent reply = new ChangeAccountStateReplyEvent(
					event.getKey(), event.getSender(), isOk, message, account);
			try {
				eventManager.sendRemoteEvent(reply);
			} catch (Exception e) {
				e.printStackTrace();
	...
	private void setAccountState(Account account, AccountState newState,
			ChangeAccountStateRequestEvent event) {
		log.info("Account:{}, old state:{} --> new state:{}", new Object[] {
				account.getId(), account.getState(), newState });
		account.setState(newState);
		ChangeAccountStateReplyEvent reply = new ChangeAccountStateReplyEvent(
				event.getKey(), event.getSender(), true, "", account);
		AccountUpdateEvent accountStateUpdate = new AccountUpdateEvent(
				account.getId(), null, account);
	...
	private void activeAccount(Account account,
		...
		try {
			...
			if (AccountState.ACTIVE.equals(account.getState())) {
				isOk = false;
				message = MessageLookup.buildEventMessage(
						ErrorMessage.ACCOUNT_ALREADY_ACTIVE,
						"Account already in ACTIVE state");
				reply = new ChangeAccountStateReplyEvent(event.getKey(),
						event.getSender(), isOk, message, account);
			} else if (terminateCheck.isOverTerminateLoss(account,
					accountSetting)) {
				isOk = false;
				message = MessageLookup
						.buildEventMessage(
								ErrorMessage.OVER_TERMINATE_LOSS,
								"Still over terminate loss, you must set terminate loss percent/value under current loss");
				reply = new ChangeAccountStateReplyEvent(event.getKey(),
						event.getSender(), isOk, message, account);
				...
				message = MessageLookup
						.buildEventMessage(
								ErrorMessage.OVER_FROZEN_LOSS,
								"Still over frozen loss, you must set frozen loss percent/value under current loss");
				reply = new ChangeAccountStateReplyEvent(event.getKey(),
						event.getSender(), isOk, message, account);
			}

			if (isOk) {
				log.info("Active Account:{}, old state:{}", account.getId(),
						account.getState());
	...
	public void processGroupManageeRequestEvent(GroupManageeRequestEvent event) {
		...
		try {
			if (!StringUtils.hasText(manager)) {
				throw new UserException("Manager is empty",
						ErrorMessage.GET_GROUP_MANAGEMENT_INFO_FAILED);
			}
			...
			// throw new
			// UserException("this user doesn't have any group",ErrorMessage.GET_GROUP_MANAGEMENT_INFO_FAILED);
			// }
		} catch (UserException e) {
			isOk = false;
			message = MessageLookup.buildEventMessage(
		...
		GroupManageeReplyEvent reply = new GroupManageeReplyEvent(
				event.getKey(), event.getSender(), isOk, message, userGroup);
		try {
			eventManager.sendRemoteEvent(reply);
		} catch (Exception e) {
	...
	public void processDeleteGroupManagementEvent(
			DeleteGroupManagementEvent event) {
		...
		if (null != userKeeper) {
			for (GroupManagement group : groupList) {
				try {
					...
					log.info("delete group : {},{}", group.getManager(),
							group.getManaged());
				} catch (UserException e) {
					isOk = false;
			...
			if (!successList.isEmpty()) {
				PmDeleteGroupManagementEvent pmEvent = new PmDeleteGroupManagementEvent(
						null, null, successList);
				try {
			...
			message = MessageLookup.buildEventMessage(
					ErrorMessage.DELETE_GROUP_MANAGEMENT_FAILED,
					"userkeeper not initialized!");
		}

		DeleteGroupManagementReplyEvent replyEvent = new DeleteGroupManagementReplyEvent(
				event.getKey(), event.getSender(), isOk, message, resultMap);
		try {
	...
	public void processCreateGroupManagementEvent(
			CreateGroupManagementEvent event) {
		List<GroupManagement> groupList = event.getGroupManagementList();
		List<GroupManagement> successList = new ArrayList<GroupManagement>();
		Map<GroupManagement, String> resultMap = new HashMap<GroupManagement, String>();
		boolean isOk = true;
		...
		if (null != userKeeper) {
			...
			if (!successList.isEmpty()) {
				PmCreateGroupManagementEvent pmEvent = new PmCreateGroupManagementEvent(
						null, null, successList);
				try {
					eventManager.sendEvent(pmEvent);
				} catch (Exception e) {
		...
		CreateGroupManagementReplyEvent replyEvent = new CreateGroupManagementReplyEvent(
				event.getKey(), event.getSender(), isOk, message, resultMap);
		try {
			eventManager.sendRemoteEvent(replyEvent);
		} catch (Exception e) {
	...
	public void processCreateAccountEvent(CreateAccountEvent event) {
		...
		if (null != userKeeper && null != accountKeeper) {
			...
			message = MessageLookup.buildEventMessage(
					ErrorMessage.CREATE_USER_FAILED,
					"System doesn't support account creation");

		}

		try {
			eventManager
					.sendRemoteEvent(new CreateAccountReplyEvent(
							event.getKey(), event.getSender(), ok, message,
							event.getTxId()));
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	...
	public void processUpdateParentOrderEvent(UpdateParentOrderEvent event) {
		...
		Account account = accountKeeper.getAccount(event.getParent()
				.getAccount());
	...
	public void processUpdateChildOrderEvent(UpdateChildOrderEvent event) {
		...
		if (null == quote) {
			eventManager.sendEvent(new QuoteSubEvent(AccountPositionManager.ID,
					null, event.getOrder().getSymbol()));
		...
		if (null != execution) {
			log.debug("Process execution: " + execution + ", "
					...
					+ event.getOrder().getStrategyId());
			try {
				Account account = accountKeeper.getAccount(execution
						.getAccount());
				positionKeeper.processExecution(execution, account);
				eventManager.sendRemoteEvent(new ExecutionUpdateEvent(execution
						.getAccount(), null, execution));
			} catch (Exception e) {
				log.error(e.getMessage(), e);
			}
		}
	...
	private void updateFxRates(Quote quote) {
		...
		if (refData != null && refData.getExchange() != null
	...
	public void processAccountSnapshotRequestEvent(
		...
		if (null == account) {
			reply = new AccountSnapshotReplyEvent(event.getKey(),
					event.getSender(), null, null, null, null, null,
					event.getTxId(), null);
			log.warn("processAccountSnapshotRequestEvent, account doesn't exist: "
					+ event.getAccountId());
		} else {
			List<OpenPosition> openPositions = positionKeeper
					.getOverallPosition(account);
			List<ClosedPosition> closedPosition = positionKeeper
					.getClosedPositions(event.getAccountId());
			List<Execution> executions = positionKeeper.getExecutions(event
					.getAccountId());
			AccountSetting accountSetting = null;
			try {
				accountSetting = accountKeeper.getAccountSetting(account
						.getId());
			} catch (AccountException e) {
				log.error(e.getMessage(), e);
			}
	...
	public void processChangeUserRoleEvent(ChangeUserRoleEvent event) {
		...
		try {
			...
			isOk = false;
			message = MessageLookup.buildEventMessage(
					ErrorMessage.CHANGE_USER_ROLE_FAILED, e.getMessage());
		...
		try {
			...
			if (isOk) {
				log.info("{} change User role:{} to {} needClearAllManagee:{}",
						new Object[] { user.getId(), user.getRole(), role,
								needClearAllManagee });
				user.setRole(role);
				...
				PmUpdateUserEvent updateUserEvent = new PmUpdateUserEvent(
						PersistenceManager.ID, null, user);
				eventManager.sendEvent(updateUserEvent);
				if (needClearAllManagee
						&& !userGroup.getNoneRecursiveManageeList().isEmpty()) {
					PmDeleteGroupManagementEvent deleteGroupManagementEvent = new PmDeleteGroupManagementEvent(
							PersistenceManager.ID, null,
							userGroup.exportGroupManagementList());
					eventManager.sendEvent(deleteGroupManagementEvent);
					userGroup.clearManageeList();
				}
			}

			ChangeUserRoleReplyEvent reply = new ChangeUserRoleReplyEvent(
					event.getKey(), event.getSender(), isOk, message, user);
			eventManager.sendRemoteEvent(reply);
	...
	public void processAllUserSnapshotRequestEvent(
			AllUserSnapshotRequestEvent event) {
		if (null == userKeeper) {
			return;
		}
			...
			public void run() {
				...
				for (i = 1; i <= allUsers.size(); i++) {
					...
					if (i % asyncSendBatch == 0 || i == allUsers.size()) {
						...
						try {
							eventManager.sendRemoteEvent(reply);
							log.info("AllUserSnapshotReplyEvent sent: "
									+ batch.size());
							Thread.sleep(asyncSendInterval);
						} catch (Exception e) {
							log.error(e.getMessage(), e);
	...
	public void processAllAccountSnapshotRequestEvent(
			AllAccountSnapshotRequestEvent event) {
		if (null == accountKeeper) {
			return;
		}
	...
	private void asyncSendPositionSnapshot(
			final AllPositionSnapshotRequestEvent event,
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				for (int i = 0; i < allAccounts.size(); i++) {
					...
					openPosition: {
						...
						if (PositionType.OpenPosition.equals(type)
								|| PositionType.All.equals(type)) {
							List<OpenPosition> tempOpList = positionKeeper
									.getOverallPosition(account);
							if (null == tempOpList || tempOpList.isEmpty()) {
								break openPosition;
					...
					closedPosition: {
						...
						if (PositionType.ClosedPosition.equals(type)
								|| PositionType.All.equals(type)) {
							List<ClosedPosition> tempCpList = positionKeeper
									.getClosedPositions(account.getId());
							...
							for (ClosedPosition cp : tempCpList) {
								...
								if (positionCount % asyncSendBatch == 0) {
									try {
										sendAllPositionReplyEvent(null,
												closedPositionList);
									} catch (Exception e) {
										log.error(e.getMessage(), e);
									}
				...
				if (positionCount != 0) {
					sendAllPositionReplyEvent(openPositionList,
							closedPositionList);
					positionCount = 0;
					openPositionList.clear();
					closedPositionList.clear();
			...
			private void sendAllPositionReplyEvent(
				...
				log.info(
						"AllPositionSnapshotReplyEvent sent: open:{}, close:{}",
						openPositionList.size(), closedPositionList.size());
			}
	...
	private void asyncSendAccountSnapshot(
			final AllAccountSnapshotRequestEvent event,
			final List<Account> allAccounts) {
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				for (i = 1; i <= allAccounts.size(); i++) {
					...
					if (i % asyncSendBatch == 0 || i == allAccounts.size()) {
						...
						try {
							eventManager.sendRemoteEvent(reply);
							log.info("AllAccountSnapshotReplyEvent sent: "
									+ batch.size());
							Thread.sleep(asyncSendInterval);
						} catch (Exception e) {
							log.error(e.getMessage(), e);
	...
	public void processAccountStateRequestEvent(AccountStateRequestEvent event) {
		...
		if (null == account) {
			...
			reply = new AccountStateReplyEvent(event.getKey(),
					event.getSender(), isOk, message, id, account.getUserId(),
					account.getState());
		}
	...
	public void processAccountSettingSnapshotRequestEvent(
			AccountSettingSnapshotRequestEvent event) {
		boolean ok = true;
		String message = null;
		AccountSetting accountSetting = null;

		try {
			accountSetting = accountKeeper.getAccountSetting(event
					.getAccountId());
		...
		AccountSettingSnapshotReplyEvent reply = new AccountSettingSnapshotReplyEvent(
				event.getKey(), event.getSender(), accountSetting, ok, message,
				event.getTxId(), getUserLiveTradingTime(accountSetting));
	...
	private ExtraEventMessageBuilder getUserLiveTradingTime(
			AccountSetting accountSetting) {

		if (null != liveTradingSetting && null != accountSetting
				&& accountSetting.isLiveTrading()) {

			ExtraEventMessageBuilder builder = new ExtraEventMessageBuilder();

			builder.putMessage(
					ExtraEventMessage.USER_STOP_LIVE_TRADING_START_TIME,
					liveTradingSetting.getUserStopLiveTradingStartTime())
					.putMessage(
							ExtraEventMessage.USER_STOP_LIVE_TRADING_END_TIME,
	...
	public void processChangeAccountSettingRequestEvent(
			ChangeAccountSettingRequestEvent event) {
		boolean ok = true;
		String message = null;
		AccountSetting accountSetting = null;
		try {
			log.info("Updating account settings: " + event.getAccountSetting());
			accountSetting = accountKeeper.setAccountSetting(event
					.getAccountSetting());
		} catch (AccountException e) {
			ok = false;
			message = MessageLookup.buildEventMessage(e.getClientMessage(),
		...
		ChangeAccountSettingReplyEvent reply = new ChangeAccountSettingReplyEvent(
				event.getKey(), event.getSender(), accountSetting, ok, message,
				getUserLiveTradingTime(accountSetting));
		try {
			eventManager.sendRemoteEvent(reply);
		...
		if (ok) {
			PmChangeAccountSettingEvent pmEvent = new PmChangeAccountSettingEvent(
					PersistenceManager.ID, null, accountSetting);
			eventManager.sendEvent(pmEvent);
		}
	}
	...
	public void processInternalResetAccountRequestEvent(
		...
		log.info("Received InternalResetAccountRequestEvent: "
				+ evt.getAccount());
		positionKeeper.resetAccount(evt.getAccount());
		try {
			eventManager.sendRemoteEvent(new ResetAccountReplyEvent(evt
					.getKey(), evt.getSender(), evt.getAccount(),
					evt.getTxId(), evt.getUserId(), evt.getMarket(), evt
							.getCoinId(), ResetAccountReplyType.LTSCORE, true,
					""));
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	...
	private void updateDynamicData() {
		try {
			...
			if (rmUpdateThrottler.check()) {
				...
				for (Account account : accounts) {
					...
					if (totalPnLCalculatorThrottler.check()) {
						log.info("Total PnL: "
								+ totalPnLCalculator.getTotalPnL()
								+ ", "
								...
								+ ", "
								+ totalPnLCalculator
										.getLiveTradingAccountValue());
					}

					if (systemSuspendValidator != null
							&& systemSuspendValidator.isSuspended()) {
						continue;
					}
					if (null != liveTradingCheckHandler
							&& accountSetting.isUserLiveTrading()) {
						if (liveTradingCheckHandler.startCheckChain(account,
								accountSetting)) {
							continue;
						}
					} else {
			...
			if (dynamicUpdateThrottler.check()) {
				...
				List<Account> accounts = accountKeeper.getDynamicJobs()
						.getJobs();
				for (Account account : accounts) {
	...
	private boolean checkDailyStopLoss(Account account,
			AccountSetting accountSetting) {
		...
		if (null != coinManager
				&& !coinManager.canCheckDailyStopLoss(account.getId())) {
			return false;
		}
		...
		if (PriceUtils.EqualLessThan(account.getDailyPnL(), -dailyStopLoss)) {
			...
			if (account.getState().equals(AccountState.ACTIVE)) {
				log.info("Account:" + account.getId() + " Daily loss: "
						+ account.getDailyPnL() + " over " + -dailyStopLoss);
			...
			TradingUtil.closeAllPositoinAndOrder(account, positionKeeper,
					eventManager, true, OrderReason.CompanyDailyStopLoss,
					riskOrderController);
			return true;
		}
	...
	private boolean checkStopLoss(Account account, AccountSetting accountSetting) {
		...
		if (null != coinManager
				&& !coinManager.canCheckPositionStopLoss(account.getId())) {
			return false;
		}
		...
		try {
			if (null != accountSetting) {
				...
				if (null == positionStopLoss
						|| PriceUtils.isZero(positionStopLoss)) {
					positionStopLoss = Default.getPositionStopLoss();
				}

				if (null != companyStopLoss
						&& !PriceUtils.isZero(companyStopLoss)) {
			...
			log.error("Check StopLoss fail at {}, {}", account.getUserId(),
					account.getId());
			return false;
		}
		...
		for (OpenPosition position : positions) {
			...
			if (PriceUtils
				...
				ClosePositionRequestEvent event = new ClosePositionRequestEvent(
						...
						position.getSymbol(), 0.0,
						OrderReason.PositionStopLoss, IdGenerator.getInstance()
								.getNextID(), true);
	...
	private boolean checkMarginCall(Account account,
		...
		if (PriceUtils.EqualLessThan(account.getCashAvailable(), 0.0)
				&& positions.size() > 0) {
			log.info("Margin call: " + account.getId() + ", "
					+ account.getCash() + ", " + account.getUrPnL() + ", "
					+ account.getCashAvailable());

			List<ParentOrder> orders = positionKeeper.getParentOrders(account
					.getId());
			...
			if (orders.size() > 0) {
				for (ParentOrder order : orders) {
					...
					try {
						...
						CancelStrategyOrderEvent cancel = new CancelStrategyOrderEvent(
								...
								OrderReason.MarginCall, false);
						eventManager.sendEvent(cancel);
						result = true;
						break;
					} catch (AccountException e) {
						log.warn("Try to cancel order but account is locked: "
								+ e.getMessage());
					}
				}
			} else {
				...
				Collections.sort(positions, new Comparator<OpenPosition>() {
					...
					public int compare(OpenPosition p1, OpenPosition p2) {
						if (PriceUtils.GreaterThan(p1.getAcPnL(), p2.getAcPnL())) {
							return 1;
						} else if (PriceUtils.LessThan(p1.getAcPnL(),
								p2.getAcPnL())) {
							return -1;
						}
				...
				for (int i = 0; i < positions.size(); i++) {
					...
					if (positionKeeper.checkAccountPositionLock(
							account.getId(), position.getSymbol())) {
					...
					double marketablePrice = QuoteUtils.getMarketablePrice(
							quote, position.getQty());
					double lossQty = FxUtils.calculateQtyFromValue(
							refDataManager, fxConverter, account.getCurrency(),
							quote.getSymbol(),
							Math.abs(account.getCashAvailable()),
							marketablePrice);

					log.info("Margin cut close position: "
							+ position.getAccount() + ", "
							+ position.getSymbol() + ", " + position.getAcPnL()
							+ ", " + position.getQty() + ", " + lossQty + ", "
							+ account.getCashAvailable() + ", " + quote);

					RefData refData = refDataManager.getRefData(quote
							.getSymbol());
					...
					if (lossQty > Default.getMarginCut()) {
						long lot = Math.max(
								Math.max((long) Default.getMarginCut(),
										refData.getLotSize()), 1);
					...
					ClosePositionRequestEvent event = new ClosePositionRequestEvent(
							position.getAccount(), null, position.getAccount(),
							position.getSymbol(), qty, OrderReason.MarginCall,
	...
	private void processDayEndTasks() {
		...
		for (Account account : list) {
			...
			eventManager.sendEvent(new PmUpdateAccountEvent(
					PersistenceManager.ID, null, copy));
	...
	public void processIndexSessionEvent(IndexSessionEvent event) {
		...
		if (list.size() > 0) {
			...
			tradeDate = first.getTradeDateByString(); // Take first
														// marketsession's trade
														// date as system's
														// trade date, but not a
														// great choice
		} else {
	...
	public void injectUsers(List<User> users) {
		...
		if (null != defaultUser) {
			eventManager.sendEvent(new PmCreateUserEvent(PersistenceManager.ID,
					null, defaultUser, new CreateUserEvent(
							PersistenceManager.ID, null, defaultUser, "TW",
							"TW", "")));
		}
	}
	...
	public void injectAccounts(List<Account> accounts) {
		...
		if (null != defaultAccount) {
			eventManager.sendEvent(new PmCreateAccountEvent(
					PersistenceManager.ID, null, defaultAccount));
		}
	}
	...
	public void injectPositions(List<OpenPosition> opens,
	...
	public void processSettlementEvent(SettlementEvent event) {
		...
		if (null == settlePrice || PriceUtils.EqualLessThan(settlePrice, 0.0)) {
			...
			if (null == quote || !QuoteUtils.validQuote(quote)) {
				log.error("processSettlementDayEvent quote is null or invalid: "
						+ symbol + ", " + quote);
				return;
			}
		}
		...
		for (Account account : accounts) {
			...
			if (!PriceUtils.isZero(position.getQty())) {
				...
				try {
					...
					log.debug("Settling position: " + position + "  with "
							+ price);
