...
public class DownStreamManager implements IPlugin {
	...
	private Map<String, IDownStreamSender> senders =
			Collections.synchronizedMap(new HashMap<String, IDownStreamSender>());
	...
	private Map<String, IDownStreamSender> specialSenders =
			Collections.synchronizedMap(new HashMap<String, IDownStreamSender>());
		...
		for (IStreamAdaptor<IDownStreamConnection> adaptor: adaptors) {
			...
			for (IDownStreamConnection connection: adaptor.getConnections()) {
				if (senders.containsKey(connection.getId())) {
					throw new DownStreamException("This connection id already exists: "
							+ connection.getId(), ErrorMessage.DOWN_STREAM_CONN_ID_EXIST);
				}
	...
	class DownStreamListener implements IDownStreamListener {
		...
		public void onState(boolean on) {
			if (!on) {
				log.warn("Down Stream connection is down: "
						+ connection.getId(), ErrorMessage.DOWN_STREAM_CONN_DOWN);
			} else {
				if (allReady()) {
					eventManager.sendEvent(new DownStreamReadyEvent(null, true));
				}
		...
		public void onOrder(ExecType execType, ChildOrder order,
			...
			UpdateChildOrderEvent event = new UpdateChildOrderEvent(
					order.getStrategyId(), execType, order, execution, message);
	...
	public IDownStreamSender getSender() throws DownStreamException {
		ArrayList<IDownStreamSender> list = new ArrayList<>();
		if (senders.size() == 0) {
			throw new DownStreamException("There are no DownStreamSender available",
					ErrorMessage.DOWN_STREAM_SENDER_NOT_AVAILABLE);
		}
