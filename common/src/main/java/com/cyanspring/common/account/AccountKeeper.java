...
public class AccountKeeper {
	...
	private ConcurrentHashMap<String, AccountSetting> accountSettings = new ConcurrentHashMap<String, AccountSetting>();
	...
	public AccountSetting getAccountSetting(String account)
			throws AccountException {
		if (!accounts.containsKey(account))
			throw new AccountException("Account id doesn't exist: " + account,
					ErrorMessage.ACCOUNT_NOT_EXIST);
	...
	private void checkLiveTradingStopLossValue(AccountSetting oldSetting,
			AccountSetting newSetting, Account account) throws AccountException {

		double comDailyStopLoss = TradingUtil.getMinValue(
				account.getStartAccountValue() * oldSetting.getFreezePercent(),
				oldSetting.getFreezeValue());

		double comPositionStopLoss = TradingUtil.getMinValue(
				account.getStartAccountValue()
						* oldSetting.getStopLossPercent(),
				oldSetting.getCompanySLValue());

		if (!PriceUtils.isZero(comDailyStopLoss)
				&& PriceUtils.GreaterThan(newSetting.getDailyStopLoss(),
						comDailyStopLoss)) {

			throw new AccountException(
					"The value you set exceeds max. daily stop loss!",
					ErrorMessage.USER_DAILY_STOP_LOSS_VALUE_EXCEEDS_COMPANY_SETTING);

		}

		if (!PriceUtils.isZero(comPositionStopLoss)
				&& PriceUtils.GreaterThan(newSetting.getStopLossValue(),
						comPositionStopLoss)) {

			throw new AccountException(
					"The value you set exceeds max. position stop loss!",
					ErrorMessage.USER_POSITION_STOP_LOSS_VALUE_EXCEEDS_COMPANY_SETTING);
		}

	}

	private void setLiveTradingStopLossValue(AccountSetting oldSetting,
			AccountSetting newSetting, Account account) throws AccountException {
		...
		double comPositionStopLoss = TradingUtil.getMinValue(
				account.getStartAccountValue()
						* oldSetting.getStopLossPercent(),
				oldSetting.getCompanySLValue());
		log.info("comDailyStopLoss:{} , comPositionStopLoss:{}",
				comDailyStopLoss, comPositionStopLoss);
		log.info(
				"oldSetting.getDailyStopLoss():{} , oldSetting.getStopLossValue():{}",
				oldSetting.getDailyStopLoss(), oldSetting.getStopLossValue());

		if (!PriceUtils.isZero(comDailyStopLoss)
				&& PriceUtils.GreaterThan(oldSetting.getDailyStopLoss(),
						comDailyStopLoss)) {
		...
		if (!PriceUtils.isZero(comPositionStopLoss)
				&& PriceUtils.GreaterThan(oldSetting.getStopLossValue(),
						comPositionStopLoss)) {
	...
	public AccountSetting setAccountSetting(AccountSetting setting)
			throws AccountException {
		if (!accounts.containsKey(setting.getId()))
			throw new AccountException("Account id doesn't exist: "
					+ setting.getId(), ErrorMessage.ACCOUNT_NOT_EXIST);
	...
	public void addAccount(Account account) {
		...
		if (null == list) {
			...
			List<Account> existing = userAccounts.putIfAbsent(
					account.getUserId(), list);
	...
	public void setupAccount(Account account) throws AccountException {
		if (accounts.containsKey(account.getId()))
			throw new AccountException("Account already exists: "
					+ account.getId(), ErrorMessage.USER_ALREADY_EXIST);
		...
		account.addMargin(account.getCash()
				* leverageManager.getLeverage(null, null));
	...
	public void injectAccountPools(List<AccountPool> accountPools) {
		for (AccountPool accountPool : accountPools) {
			accounts.get(accountPool.getAccount()).add(accountPool);
		}
	}
	...
	public List<Account> getAccountsByRouter(String router)
			throws AccountException {
