...
public class ErrorSchema {
	...
	public static void build() throws Exception {
		...
		createMsg(207, "EN",
				"Display quantity can not be empty or 0 for Iceberg stratgy");
		...
		createMsg(209, "EN", "Price can not be empty or 0 for Sniper stratgy");
		...
		createMsg(215, "EN", "Can't find symbol in refdata");
		...
		createMsg(218, "CN", "不能通过数量份额验证确定股票");
		createMsg(218, "EN",
		...
		createMsg(236, "EN", "Fix side not handdled");
		...
		createMsg(5100, "CN", "创建 证券交易账号失败");
		createMsg(5100, "EN", "Failed to create new ExchangeAccount");

		createMsg(5101, "CN", "请先删除关联的交易分账号");
		createMsg(5101, "EN",
				"Please delete the related ExchangeSubAccount(s) first");
		...
		createMsg(5204, "CN", "请先删除该用户对应的交易分账号");
		createMsg(5204, "EN",
				"Please delete the ExchangeSubAccount(s) of this user");

		// Pool & SubPool: 5300 -> 5399
		createMsg(5300, "CN", "股票池中股票数量不能大于总池中的数量");
		createMsg(5300, "EN",
				"Quantity of instrument in SubPool can't be bigger than that in total Pool");

		// ExchangeSubAccount: 5400 -> 5499
		createMsg(5400, "CN", "分账号已经存在");
		createMsg(5400, "EN", "ExchangeSubAccount already exists");

		createMsg(5401, "CN", "无效的分账号");
		createMsg(5401, "EN", "Invalid ExchangeSubAccount");

		createMsg(5402, "CN", "超出限定股票数量");
		createMsg(5402, "EN", "Quantity is over the limit");

		createMsg(5403, "CN", "请先删除关联的股票池");
		createMsg(5403, "EN", "Please delete its' SubPool(s) first");

		// Instrument: 5500 -> 5599
		createMsg(5500, "CN", "该股票已经存在");
