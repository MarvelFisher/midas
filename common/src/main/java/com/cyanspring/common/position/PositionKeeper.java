...
public class PositionKeeper {
	...
	private ConcurrentHashMap<String, String> accounts = new ConcurrentHashMap<String, String>(); // for
																									// synchronization
	private ConcurrentHashMap<String, Map<String, List<OpenPosition>>> accountPositions = new ConcurrentHashMap<String, Map<String, List<OpenPosition>>>();
	private ConcurrentHashMap<String, List<ClosedPosition>> closedPositions = new ConcurrentHashMap<String, List<ClosedPosition>>();
	private ConcurrentHashMap<String, List<Execution>> executions = new ConcurrentHashMap<String, List<Execution>>();
	private ConcurrentHashMap<String, Map<String, Map<String, ParentOrder>>> parentOrders = // keys:
	...
	public void processParentOrder(ParentOrder order, Account account) {
		...
		Map<String, Map<String, ParentOrder>> accountMap = parentOrders
				.get(order.getAccount());
		if (null == accountMap) {
			accountMap = new ConcurrentHashMap<String, Map<String, ParentOrder>>();
			Map<String, Map<String, ParentOrder>> existing = parentOrders
					.putIfAbsent(order.getAccount(), accountMap);
			accountMap = existing == null ? accountMap : existing;
		}

		synchronized (getSyncAccount(order.getAccount())) {
			Map<String, ParentOrder> symbolMap = accountMap.get(order
					.getSymbol());
			if (null == symbolMap) {
			...
			if (order.getOrdStatus().isCompleted()
					|| order.getState() == StrategyState.Terminated) {
	...
	private OpenPosition createOpenPosition(double qty, Execution execution,
			Account account) {
		...
		if (null != refDataManager) {
			margin = FxUtils.convertPositionToCurrency(refDataManager,
					fxConverter, account.getCurrency(), execution.getSymbol(),
					qty, execution.getPrice());
			RefData refData = refDataManager.getRefData(execution.getSymbol());
			double leverage = leverageManager.getLeverage(refData,
					accountSetting);
			margin /= leverage;
			log.debug("Open position margin: " + qty + ", " + margin + ", "
					+ leverage);
	...
	public void processExecution(Execution execution, Account account)
			throws PositionException {
		if (null == execution.getAccount()) {
			throw new PositionException("Execution has no account: "
					+ execution, ErrorMessage.ACCOUNT_NOT_EXIST);
		}
		...
		Map<String, Map<String, ParentOrder>> accountOrders = parentOrders
				.get(execution.getAccount());
		if (null != accountOrders) {
			Map<String, ParentOrder> symbolOrders = accountOrders.get(execution
					.getSymbol());
			if (null != symbolOrders) {
				...
				if (null != parentOrder) {
					...
					if (PriceUtils.EqualGreaterThan(parentOrder.getCumQty(),
							parentOrder.getQuantity())) { // fully filled
						symbolOrders.remove(execution.getParentOrderId());
						log.debug("Remove fully filled parentOrder: "
		...
		if (!parentOrderUdpated) {
			log.warn("Cant location parent order: "
					+ execution.getParentOrderId());
		}
		...
		if (null == symbolPositions) {
			...
			Map<String, List<OpenPosition>> existing = this.accountPositions
					.putIfAbsent(execution.getAccount(), symbolPositions);
			symbolPositions = existing == null ? symbolPositions : existing;
		}

		synchronized (getSyncAccount(execution.getAccount())) {
			List<OpenPosition> list = symbolPositions
					.get(execution.getSymbol());
			if (null == list) {
			...
			if (list.size() <= 0) { // no existing position, just add it
				OpenPosition position = createOpenPosition(
						execution.getQuantity(), execution, account);
				list.add(position);
				this.notifyUpdateDetailOpenPosition(position);
			} else {

				OpenPosition overallPosition = getOverallPosition(list,
						account, execution.getSymbol());
				...
				if (overallPosition.oppositePosition(execPos)) {
					...
					while (it.hasNext() && !PriceUtils.isZero(execPos)) {
						...
						if (PriceUtils.EqualGreaterThan(Math.abs(execPos),
								Math.abs(pos.getQty()))) {
							...
							transferToClosedPositions(transfer, execution,
									account);
							execPos = 0;
					...
					if (PriceUtils.GreaterThan(Math.abs(execPos), 0)) {
						OpenPosition position = createOpenPosition(
								Math.abs(execPos), execution, account);
						list.add(position);
						this.notifyUpdateDetailOpenPosition(position);
					}
			...
			if (!PriceUtils.isZero(commissionManager.getCommission(refData,
					accountSetting, execution))) {
				double value = FxUtils.convertPositionToCurrency(
						refDataManager, fxConverter, account.getCurrency(),
						execution.getSymbol(), execution.getQuantity(),
						execution.getPrice());
				double commision = commissionManager.getCommission(refData,
						accountSetting, value, execution);
				account.updatePnL(-commision);
			}

			updateAccountDynamicData(account);

			OpenPosition update = getOverallPosition(account,
					execution.getSymbol());
			notifyOpenPositionUpdate(update);
		...
		if (parentOrder != null
				&& checkAccountPositionLock(parentOrder.getAccount(),
						parentOrder.getSymbol())
				&& parentOrder.getOrdStatus().equals(OrdStatus.FILLED)
				&& PriceUtils.Equal(parentOrder.getCumQty(),
						parentOrder.getQuantity())) {
			unlockAccountPosition(parentOrder.getId());
			log.debug("Close position action completed ex: "
					+ parentOrder.getAccount() + ", " + parentOrder.getSymbol()
					+ ", " + parentOrder.getId());
		}
	...
	public List<ParentOrder> getParentOrders(String account) {
		...
		Map<String, Map<String, ParentOrder>> symbolMap = parentOrders
				.get(account);
		if (null != symbolMap) {
	...
	protected void transferToClosedPositions(OpenPosition position,
		...
		if (null == list) {
			...
			List<ClosedPosition> existing = closedPositions.putIfAbsent(
					position.getAccount(), list);
			list = existing == null ? list : existing;
		}

		ClosedPosition pos = ClosedPosition.create(refDataManager, fxConverter,
				position, execution, account);
		list.add(pos);

		account.updatePnL(pos.getAcPnL());
		account.setMarginHeld(account.getMarginHeld()
				- Math.abs(position.getMargin()));
	...
	private void addExecution(Execution execution) {
		...
		if (null == list) {
			...
			List<Execution> prev = executions.putIfAbsent(
					execution.getAccount(), list);
	...
	public List<OpenPosition> getOverallPosition(Account account) {
		...
		Map<String, List<OpenPosition>> ap = accountPositions.get(account
				.getId());
		if (null == ap) {
			return result;
		}
	...
	public OpenPosition getOverallPosition(Account account, String symbol) {
		OpenPosition result = new OpenPosition(account.getUserId(),
				account.getId(), symbol, 0, 0, 0);
		Map<String, List<OpenPosition>> ap = accountPositions.get(account
				.getId());
		if (null == ap) {
			return result;
			...
			try {
				...
				if (null != quoteFeeder) {
					...
					if (null != quote && null != pos) {
						double price = QuoteUtils.getPnlPrice(quote,
								pos.getQty(), useMid);
						...
						if (PriceUtils.validPrice(price)) {
							double pnl = FxUtils.calculatePnL(refDataManager,
									pos.getSymbol(), pos.getQty(),
									(price - pos.getPrice()));
							pos.setPnL(pnl);
							double urPnL = FxUtils.convertPnLToCurrency(
									refDataManager, fxConverter,
						...
						if (PriceUtils.validPrice(lastPrice)) {
							double lastPnL = FxUtils.calculatePnL(
									refDataManager, pos.getSymbol(),
									pos.getQty(), (lastPrice - pos.getPrice()));
							pos.setLastPnL(lastPnL);
							double acLastPnL = FxUtils.convertPnLToCurrency(
									refDataManager, fxConverter,
									account.getCurrency(), quote.getSymbol(),
									pos.getLastPnL());
						...
						/*
						 * if(pos.getPnL() > pos.getLastPnL() || pos.getAcPnL()
						 * > pos.getAcLastPnL()){ log.info("Strange LastPnl - "
						 * + pos.getQty() + ", lastPrice: " + lastPrice +
						 * ", price: " + price + ", position price: " +
						 * pos.getPrice() + ", Pnl: " + pos.getPnL() +
						 * ", AcPnl:" + pos.getAcPnL() + ", lastPnl: " +
						 * pos.getLastPnL() + ", AcLastPnl: " +
						 * pos.getAcLastPnL() + ", bid:" + quote.getBid() +
						 * ", ask: " + quote.getAsk()); }
						 */
	...
	private OpenPosition getOverallPosition(List<OpenPosition> list,
			Account account, String symbol) throws PositionException {
		...
		if (refData != null && refData.getTradableDays() >= 1
				&& Default.getSettlementDays() == 1) {
			if (availableQty > 0) {
		...
		OpenPosition result = new OpenPosition(list.get(0).getUser(), list.get(
				0).getAccount(), list.get(0).getSymbol(), qty, price, margin);
		result.setPnL(PnL);
	...
	private void checkOverallPosition(List<OpenPosition> list)
			throws PositionException {
		...
		for (OpenPosition pos : list) {
			if (!pos.getAccount().equals(first.getAccount())) {
				throw new PositionException(
						"Position list contains different account: " + first
								+ " vs " + pos,
						ErrorMessage.POSITION_CONTAINS_DIFF_ACCOUNT);
			}

			if (!pos.getSymbol().equals(first.getSymbol())) {
				throw new PositionException(
						"Position list contains different symbol: " + first
								+ " vs " + pos,
						ErrorMessage.POSITION_CONTAINS_DIFF_SYMBOL);
			}

			if (first.oppositePosition(pos)) {
				throw new PositionException(
						"Position list contains different side: " + first
								+ " vs " + pos,
						ErrorMessage.POSITION_CONTAINS_DIFF_SIDE);
			}

			if (PriceUtils.isZero(pos.getQty())) {
				throw new PositionException("Position list contains 0: " + pos,
						ErrorMessage.POSITION_CONTAINS_ZERO);
			}
		}
	}
	...
	private List<String> getCombinedSymbolList(String account) {
		...
		Map<String, List<OpenPosition>> symbolPositions = accountPositions
				.get(account);
		Map<String, Map<String, ParentOrder>> symbolOpenOrders = parentOrders
				.get(account);
	...
	public void updateAccountDynamicData(Account account) {
		...
		synchronized (getSyncAccount(account.getId())) {
			// since margin is contributed by both open position and open
			// orders, we need
				...
				if (null != symbolPositions) {
					...
					if (null != list) {
						...
						for (OpenPosition position : list) {
							...
							double pnl = FxUtils.calculatePnL(refDataManager,
									position.getSymbol(), position.getQty(),
									(price - position.getPrice()));
							position.setPnL(pnl);
							double lastPnL = FxUtils.calculatePnL(
									refDataManager, position.getSymbol(),
									position.getQty(),
						...
						OpenPosition overallPosition = getOverallPosition(
								account, symbol);
						accountUrPnL += overallPosition.getAcPnL();
						accountUrLastPnl += overallPosition.getAcLastPnL();
						leverageUrPnL += overallPosition.getAcPnL()
								* (1 - 1 / lev);
					}
				}

				double value = getMarginValueByAccountAndSymbol(account,
						symbol, quote, lev);
			...
			double accountMargin = (account.getCash() + account.getUrPnL())
					* leverageManager.getLeverage(null, accountSetting);
			if (PriceUtils.GreaterThan(accountSetting.getMargin(), 0)) {
				accountMargin = (account.getCash() + account.getUrPnL())
						* accountSetting.getMargin();
			}
			account.setMargin(accountMargin - marginValue);
			account.setCashDeduct(account.getCash() - account.getMarginHeld()
					+ leverageUrPnL);
			account.setCashAvailable(account.getCash() - marginHeld
					+ leverageUrPnL);
		}
	}
	...
	public void injectOpenPositions(List<OpenPosition> positions) {
		for (OpenPosition position : positions) {
			Map<String, List<OpenPosition>> symbolPositions = accountPositions
					.get(position.getAccount());
			if (null == symbolPositions) {
				symbolPositions = new ConcurrentHashMap<String, List<OpenPosition>>();
				this.accountPositions.put(position.getAccount(),
						symbolPositions);
			}
	...
	public void injectClosedPositions(List<ClosedPosition> positions) {
		for (ClosedPosition position : positions) {
			List<ClosedPosition> list = closedPositions.get(position
					.getAccount());
			if (null == list) {
				list = new LinkedList<ClosedPosition>();
	...
	public void resetMarginHeld() {
		...
		for (Entry<String, Map<String, List<OpenPosition>>> entry : accountPositions
				.entrySet()) {
			if (null == entry) {
				continue;
			}
			...
			try {
				accountSetting = accountKeeper
						.getAccountSetting(entry.getKey());
			} catch (AccountException e) {
				log.error(e.getMessage(), e);
			}
			...
			for (List<OpenPosition> list : entry.getValue().values()) {
				...
				double total = 0.0;
				for (OpenPosition position : list) {
					double margin = FxUtils.convertPositionToCurrency(
							refDataManager, fxConverter, account.getCurrency(),
							position.getSymbol(), Math.abs(position.getQty()),
							position.getPrice());
					RefData refData = refDataManager.getRefData(position
							.getSymbol());
					double leverage = leverageManager.getLeverage(refData,
							accountSetting);
					margin /= leverage;
	...
	public void updateAccountOpenPosition(String account, String symbol,
			double price) throws AccountException {
		...
		for (OpenPosition position : list) {
			...
			double qPrice = QuoteUtils.getPnlPrice(quote, position.getQty(),
					useMid);
			...
			RefData refData = refDataManager.getRefData(symbol);
			double margin = FxUtils.convertPositionToCurrency(refDataManager,
					fxConverter, acc.getCurrency(), position.getSymbol(),
					Math.abs(position.getQty()), price);
			double leverage = leverageManager.getLeverage(refData, settings);
			margin /= leverage;
			acc.setMarginHeld(acc.getMarginHeld() - position.getMargin()
					+ margin);
			position.setMargin(margin);
			position.setPrice(price);

			double pnl = FxUtils.calculatePnL(refDataManager,
					position.getSymbol(), position.getQty(), (qPrice - price));
			position.setPnL(pnl);
			double urPnL = FxUtils.convertPnLToCurrency(refDataManager,
					fxConverter, acc.getCurrency(), quote.getSymbol(),
					position.getPnL());
