...
public abstract class Strategy implements IStrategy, IAsyncExecuteEventListener {
	...
	private long execPendingTimeOut = 20000; 
	...
	public void logInfo(String message) {
		...
		container.sendRemoteEvent(new StrategyLogEvent(getId(), null, LogType.Info, clientMessage(message), getAccount()));
	}
	public void logWarn(String message) {
		getLog().warn( "[" + getId() + "] " + message);
		container.sendRemoteEvent(new StrategyLogEvent(getId(), null, LogType.Warn, clientMessage(message), getAccount()));
		setAlert(AlertType.WARNING, message);
	}
	
	public void logError(String message) {
		getLog().error( "[" + getId() + "] " + message);
		container.sendRemoteEvent(new StrategyLogEvent(getId(), null, LogType.Error, clientMessage(message), getAccount()));
	...
	protected boolean hasPendingChildOrder() {
		for(ChildOrder child: childOrders.values()) {
			if(child.getOrdStatus().isPending() || child.getType() == ExchangeOrderType.MARKET) {
				return true;
			}
		}
		return false;
	...
	protected void executeInstructions(List<ExecutionInstruction> eis) {
		if(eis != null && eis.size()>0) {
			...
			executionManager.processExecutionInstructionEvent(event);
	...
	public void setQuoteRequired(boolean checkQuote) {
		this.quoteRequired = checkQuote;
	}

	public boolean isTimerEventRequired() {
		return timerEventRequired;
	}

	public void setTimerEventRequired(boolean timerEventRequired) {
		this.timerEventRequired = timerEventRequired;
	}
