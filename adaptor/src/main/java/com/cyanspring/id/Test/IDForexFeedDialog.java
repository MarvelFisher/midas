...
public class IDForexFeedDialog extends JFrame {
	...
	//static IDForexFeedDialog mainFrame;
	...
	private JSplitPane splitPaneV, m_splitPaneV2;
	...
	public IDForexFeedDialog(IFrameClose caller, boolean bUsingMsg) {
		callback = caller;
		isUsingMsg = bUsingMsg;
		setBackground(Color.LIGHT_GRAY);

		defaultPanel = new JPanel((LayoutManager) new FlowLayout(
				FlowLayout.LEFT));
		defaultPanel
		...
		inputPanel.setPreferredSize(new Dimension(FRAME_WIDTH,
				FRAME_UI_01_HEIGHT - 30));

		m_splitPaneV2 = new JSplitPane(JSplitPane.VERTICAL_SPLIT, defaultPanel,
				inputPanel);
		FrameUtil.flattenSplitPane(m_splitPaneV2);
		...
		if (isUsingMsg) {
			...
			final JSplitPane splitPaneV3 = new JSplitPane(
					JSplitPane.VERTICAL_SPLIT, scrollMsg, scrollLog);

			splitPaneV = new JSplitPane(JSplitPane.VERTICAL_SPLIT, m_splitPaneV2,
					splitPaneV3);
		...
		ImageIcon icon2 = new ImageIcon("Image/idforex.jpg");
	...
	public static IDForexFeedDialog Instance(IFrameClose caller,
			String strTitle, boolean bUsingMsg) {
		...
		final IDForexFeedDialog mainFrame = new IDForexFeedDialog(caller, bUsingMsg);
