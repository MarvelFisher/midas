...
//				System.out.flush();
	...
	public void calculateMessageFlow(int rBytes){
		...
		{
			bufLenMin = rBytes;
			log.info("WindC-minimal recv len from wind : " + bufLenMin);
		} else {
			if(bufLenMin == 0) {
				bufLenMin = rBytes;
				log.info("WindC-first time recv len from wind : " + bufLenMin);
			}
		}

		if(bufLenMax < rBytes) {
			bufLenMax = rBytes;
			log.info("WindC-maximal recv len from wind : " + bufLenMax);
		}
