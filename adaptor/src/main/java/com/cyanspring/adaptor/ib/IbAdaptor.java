...
public class IbAdaptor implements EWrapper, IMarketDataAdaptor,
	...
	private Thread delThread = new Thread(new Runnable() {
		@Override
		public void run() {
	...
	public boolean getState() {
		return !cancelOpenOrders ? clientSocket.isConnected()
				: !cancelOpenOrders;
	}
	...
	class DownStreamConnection implements IDownStreamConnection {
		...
		public void setState(boolean state) {
			if (null != downStreamSender)
				downStreamListener.onState(state);
		}
	}
	...
	synchronized public void error(int id, int errorCode, String errorMsg) {
		...
		if (null != order && order.getOrdStatus().isPending()
				&& errorCode != 202
	...
	synchronized public void orderStatus(int orderId, String status,
		...
		log.info("delOrdersLst size: " + delOrdersLst.size());
		// if(delOrdersLst.isEmpty()){
		// cancelOpenOrders = false;
		// }
