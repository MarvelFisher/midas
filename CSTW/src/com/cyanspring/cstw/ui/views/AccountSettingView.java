...
public class AccountSettingView extends ViewPart implements IAsyncEventListener {
	...
	public void createPartControl(Composite parent) {
		...
		Business.getInstance().getEventManager()
				.subscribe(ChangeAccountSettingReplyEvent.class, this);
	...
	public void dispose() {
		Business.getInstance().getEventManager()
				.unsubscribe(AccountSettingSnapshotReplyEvent.class, this);
		Business.getInstance().getEventManager()
				.unsubscribe(AccountSelectionLocalEvent.class, this);
		Business.getInstance().getEventManager()
				.unsubscribe(ChangeAccountSettingReplyEvent.class, this);
		super.dispose();
	}
			...
			try {
				...
				if (isNumberType(strOld) && !isNumberType(strNew)) {
					MessageBox messageBox = new MessageBox(
							composite.getShell(), SWT.ERROR);
					messageBox.setText("Error Type");
					messageBox.setMessage(pair.key + " type error : " + strOld
							+ " -> " + strNew);
					messageBox.open();
					return;
				}
		...
		try {
			if (changedFields.size() > 0
					&& MessageDialog.openConfirm(
							viewer.getControl().getShell(), "Are you sure?",
							sb.toString())) {
				AccountSetting changes = new AccountSetting(id);
				for (KeyValue pair : changedFields) {
					Object oldValue = oldFields.get(pair.key);
					Object newValue = BeanHolder
							.getInstance()
							.getDataConverter()
							.fromString(
									oldValue == null ? String.class
											: oldValue.getClass(), pair.key,
									pair.value.toString());
					changes.put(pair.key, newValue);
				}
				changes.put(AccountSettingType.ID.value(), id);
				sendRemoteEvent(new ChangeAccountSettingRequestEvent(ID,
						Business.getBusinessService().getFirstServer(), changes));
				sendRemoteEvent(new AccountSettingSnapshotRequestEvent(ID,
						Business.getBusinessService().getFirstServer(), id,
						null));
			}
		} catch (DataConvertException e) {
			log.error(e.getMessage(), e);
	...
	public void onEvent(final AsyncEvent event) {
		if (event instanceof AccountSelectionLocalEvent) {
			...
			AccountSettingSnapshotRequestEvent request = new AccountSettingSnapshotRequestEvent(
			...
			if (!editMode)
				displayObject(((AccountSettingSnapshotReplyEvent) event)
						.getAccountSetting().getFields());
