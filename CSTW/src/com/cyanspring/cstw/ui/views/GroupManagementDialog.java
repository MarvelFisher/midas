...
public class GroupManagementDialog extends Dialog implements
		IAsyncEventListener {
	...
	private final Color RISK_MANAGER_COLOR = new Color(Display.getCurrent(),
			RED);
	...
	protected GroupManagementDialog(Shell parentShell, String accountId,
	...
	protected Control createDialogArea(Composite parent) {
		...
		manageeListView = new TableViewer(container, SWT.BORDER
				| SWT.FULL_SELECTION | SWT.SCROLLBAR_OVERLAY);
		...
		nonManageeListView = new TableViewer(container, SWT.BORDER
				| SWT.FULL_SELECTION | SWT.SCROLLBAR_OVERLAY);
		...
		messageTableView = new TableViewer(container, SWT.BORDER
				| SWT.FULL_SELECTION);
		messageTable = messageTableView.getTable();
		GridData gd_message = new GridData(SWT.FILL, SWT.CENTER, true, false,
				3, 1);
	...
	private void sendGroupManageeRequestEvent() {
		GroupManageeRequestEvent event = new GroupManageeRequestEvent(ID,
				Business.getBusinessService().getFirstServer(), accountId);
		sendRemoteEvent(event);
	}

	protected void doAssign() {
		IStructuredSelection sel = (IStructuredSelection) nonManageeListView
				.getSelection();
		if (null == sel || null == sel.getFirstElement()
				|| null == sel.getFirstElement())
			return;
	...
	protected void doDeAssign() {
		IStructuredSelection sel = (IStructuredSelection) manageeListView
				.getSelection();
		if (null == sel || null == sel.getFirstElement()
				|| null == sel.getFirstElement())
			return;
	...
	protected void sendManageeSet() {
		...
		java.util.List<UserGroup> originManageeList = userGroup
		...
		for (UserGroup ug : originManageeList) {
			...
			if (!isExist)
				delList.add(new GroupManagement(userGroup.getUser(), ug
						.getUser()));
		}
		...
		if (!addList.isEmpty()) {
			CreateGroupManagementEvent addEvent = new CreateGroupManagementEvent(
					ID, Business.getBusinessService().getFirstServer(), addList);
			sendRemoteEvent(addEvent);
		}

		if (!delList.isEmpty()) {
			DeleteGroupManagementEvent delEvent = new DeleteGroupManagementEvent(
					ID, Business.getBusinessService().getFirstServer(), delList);
	...
	private void showConfirmResultOnLabel(
			Map<GroupManagement, String> resultMap, boolean isAdd) {
		...
		for (Entry<GroupManagement, String> entry : entrys) {
			...
			if (isAdd)
				pushMessage("Add Managee:" + gm.getManaged() + "  Result:"
						+ msg);
			else
				pushMessage("Delete Managee:" + gm.getManaged() + "  Result:"
						+ msg);
		}
		refreshMessage();
	}
	...
	private void showManageeList() {
		...
		try {
			...
			java.util.List<UserGroup> manageeGroupList = userGroup
					.getNoneRecursiveManageeList();
			...
			parent.getDisplay().syncExec(new Runnable() {
				...
				@Override
				public void run() {
					nonManageeListView.add(tempList.toArray(new String[tempList
							.size()]));
					manageeListView.add(manageeList
							.toArray(new String[manageeList.size()]));
					setRoleColor(nonManageeListView);
					setRoleColor(manageeListView);
				}
			});
