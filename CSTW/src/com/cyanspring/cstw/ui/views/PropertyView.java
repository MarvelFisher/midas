...
public class PropertyView extends ViewPart implements IAsyncEventListener {
	...
	public void createPartControl(Composite parent) {
		...
		Business.getInstance()
				.getEventManager()
				.subscribe(SingleInstrumentStrategySelectionLocalEvent.class,
						this);
		Business.getInstance().getEventManager()
				.subscribe(AmendParentOrderReplyEvent.class, this);
		Business.getInstance().getEventManager()
				.subscribe(ParentOrderUpdateEvent.class, this);
		Business.getInstance().getEventManager()
				.subscribe(MultiInstrumentStrategyUpdateEvent.class, this);
		Business.getInstance()
				.getEventManager()
				.subscribe(MultiInstrumentStrategySelectionLocalEvent.class,
						this);
		Business.getInstance().getEventManager()
				.subscribe(InstrumentSelectionLocalEvent.class, this);
		Business.getInstance().getEventManager()
				.subscribe(SignalSelectionLocalEvent.class, this);
		Business.getInstance().getEventManager()
	...
	public void dispose() {
		...
		Business.getInstance()
				.getEventManager()
				.unsubscribe(SingleInstrumentStrategySelectionLocalEvent.class,
						this);
		Business.getInstance().getEventManager()
				.unsubscribe(AmendParentOrderReplyEvent.class, this);
		Business.getInstance().getEventManager()
				.unsubscribe(ParentOrderUpdateEvent.class, this);
		Business.getInstance().getEventManager()
				.unsubscribe(MultiInstrumentStrategyUpdateEvent.class, this);
		Business.getInstance()
				.getEventManager()
				.unsubscribe(MultiInstrumentStrategySelectionLocalEvent.class,
						this);
		Business.getInstance().getEventManager()
				.unsubscribe(InstrumentSelectionLocalEvent.class, this);
		Business.getInstance().getEventManager()
				.unsubscribe(SignalSelectionLocalEvent.class, this);
		Business.getInstance().getEventManager()
	...
	private void confirmChange() {
		...
		for (KeyValue pair : changedFields) {
			...
			String strNew = "[new]";
			try {
				strOld = BeanHolder.getInstance().getDataConverter()
						.toString(pair.key, oldFields.get(pair.key));
				strNew = BeanHolder.getInstance().getDataConverter()
		...
		if (changedFields.size() > 0
			...
			if (clazz.equals(SingleOrderStrategySelectionLocalEvent.class)) {
				event = new AmendParentOrderEvent(id, server, id, changes, null);
					...
					.equals(MultiInstrumentStrategySelectionLocalEvent.class)) {
				event = new AmendMultiInstrumentStrategyEvent(id, server,
						changes, null, null);
			} else if (clazz.equals(InstrumentSelectionLocalEvent.class)) {
				String strategyId = (String) oldFields
						.get(OrderField.STRATEGY_ID.value());
				changes.put(OrderField.ID.value(), id);
				List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
				list.add(changes);
				event = new AmendMultiInstrumentStrategyEvent(strategyId,
						server, null, list, null);
	...
	private void setEditMode(boolean editMode) {
		...
		if (editMode) {
			...
			ImageDescriptor imageDesc = imageRegistry
					.getDescriptor(ImageID.NONE_EDIT_ICON.toString());
			editAction.setImageDescriptor(imageDesc);
	...
	public void onEvent(final AsyncEvent event) {
		if (event instanceof ObjectSelectionLocalEvent) {
			...
			objectId = (String) selectionEvent.getData().get(
					OrderField.ID.value());
			...
			if (clazz.equals(MultiInstrumentStrategySelectionLocalEvent.class)) {
				...
				Instrument instr = update.getStrategyData().getInstrumentData()
						.get(objectId);
				if (null == instr)
					return;
			...
			String symbol = signalEvent.getSignal().get(String.class,
					OrderField.ID.value());
			...
			if (!reply.isOk())
				Business.getInstance()
						.getEventManager()
						.sendEvent(
								new StrategyLogEvent(reply.getKey(), null,
										LogType.Warn, "Amend order failed: "
												+ reply.getMessage(),
										CSTWSession.getInstance()
												.getAccountId()));
		} else {
