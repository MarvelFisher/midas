...
public class ExecutionView extends ViewPart implements IAsyncEventListener {
	...
	public void createPartControl(Composite parent) {
		...
		Business.getInstance().getEventManager().subscribe(AllExecutionSnapshotReplyEvent.class, this);
		...
		AllExecutionSnapshotRequestEvent event = new AllExecutionSnapshotRequestEvent(ID,Business.getInstance().getFirstServer(),id);
		try {
	...
	public void dispose() {
		super.dispose();
		Business.getInstance().getEventManager().unsubscribe(ChildOrderUpdateEvent.class, this);
		Business.getInstance().getEventManager().unsubscribe(AllExecutionSnapshotReplyEvent.class, this);
	...
	public void onEvent(final AsyncEvent e) {
		if (e instanceof ChildOrderUpdateEvent) {
			...
			if(!event.isOk()){
				log.warn("process AllExecutionSnapshotReplyEvent fail:{}",event.getMessage());
				return;
			}
